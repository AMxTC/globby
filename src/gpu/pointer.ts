...
import { buildObjectMenu } from "../components/menus/objectMenu";
import {
  computeFaceDrag,
  detectFace,
  type FaceDragParams,
...
import {
  setRawCursor,
  makePushPullArrowCursor,
  makeHoverArrowCursor,
} from "../lib/cursors";
...
function pushPullPolyEdge(
  ...
  const pushedA: [number, number] = [
    startVerts[ei][0] + nx * localDelta,
    startVerts[ei][1] + nz * localDelta,
  ];
  const pushedB: [number, number] = [
    startVerts[j][0] + nx * localDelta,
    startVerts[j][1] + nz * localDelta,
  ];
  // Direction of the pushed edge (same as original edge direction)
  const edgeDir: [number, number] = [
    startVerts[j][0] - startVerts[ei][0],
    startVerts[j][1] - startVerts[ei][1],
  ];

  // Previous edge: prev -> ei. Its direction stays fixed.
  const prev = (ei - 1 + count) % count;
  const prevDir: [number, number] = [
    startVerts[ei][0] - startVerts[prev][0],
    startVerts[ei][1] - startVerts[prev][1],
  ];
  ...
  const nextDir: [number, number] = [
    startVerts[next][0] - startVerts[j][0],
    startVerts[next][1] - startVerts[j][1],
  ];
...
export function setupPointer(
  ...
  // Track right-click drag to suppress context menu after orbiting
  let rightDownPos: { x: number; y: number } | null = null;
  let rightDragged = false;
  const RIGHT_DRAG_THRESHOLD = 3; // px

  /** Check if a screen point (relative to canvas) is inside the union AABB of selected shapes. */
  function isOverSelectedAABB(clientX: number, clientY: number): boolean {
    ...
    for (const shape of sceneState.shapes) {
      ...
      if (!sr) continue;
      if (sr.minX < sMinX) sMinX = sr.minX;
      if (sr.minY < sMinY) sMinY = sr.minY;
      if (sr.maxX > sMaxX) sMaxX = sr.maxX;
      if (sr.maxY > sMaxY) sMaxY = sr.maxY;
    }
    return mx >= sMinX && mx <= sMaxX && my >= sMinY && my <= sMaxY;
  }
    ...
    pickChain = pickChain
      .then(() => {
        ...
        return renderer.pickWorldPos(pixelX, pixelY).then((result) => {
          ...
          setRawCursor(
            makeHoverArrowCursor(
              cursorAngle(shape.position, axisDir, face.negative),
            ),
          );
        });
      })
  ...
  function axisDirToScreenAngle(shapePos: Vec3, axisDir: Vec3): number {
    ...
    const [sx1, sy1] = worldToScreen(
      [
        shapePos[0] + axisDir[0],
        shapePos[1] + axisDir[1],
        shapePos[2] + axisDir[2],
      ],
      _vpMat,
      cw,
      ch,
  ...
  function onPointerDown(e: PointerEvent) {
    ...
    if (tool === "select") {
      ...
      const renderer = getRenderer();
      if (renderer) {
        const { px, py } = cssRectToDevicePixels(
          e.clientX,
          e.clientY,
          0,
          0,
    ...
    if (tool === "pushpull") {
      ...
      pickChain = pickChain
        .then(() => renderer.pickWorldPos(pixelX, pixelY))
        .then((result) => {
    ...
    if (tool === "polygon") {
      ...
      const penPlaneHit =
        sceneState.penVertices.length > 0
      ...
      if (renderer) {
        renderer.pickWorldPos(pixelX, pixelY).then((result) => {
          ...
          if (
            ...
            !isShapeTool(sceneState.activeTool)
          )
            return;
  ...
  function onPointerMove(e: PointerEvent) {
    ...
    if (pushPullDrag) {
      ...
      const currentT = projectRayOnAxisDir(
        ...
        e.clientX,
        e.clientY,
        pushPullDrag.startPos,
        pushPullDrag.axisDir,
      );
      const delta = currentT - pushPullDrag.startT;

      const shape = sceneState.shapes.find(
        (s) => s.id === pushPullDrag!.shapeId,
      );
      if (!shape) return;

      // Polygon edge drag: offset edge along normal, constrain adjacent edges
      if (
        pushPullDrag.edgeIdx !== undefined &&
        pushPullDrag.edgeNormal2D &&
        pushPullDrag.startVertices
      ) {
        const localDelta = delta / shape.scale;
        const [nx, nz] = pushPullDrag.edgeNormal2D;
        const newVerts = pushPullPolyEdge(
          pushPullDrag.startVertices,
          pushPullDrag.edgeIdx,
          nx,
          nz,
          localDelta,
        );
    ...
    if (shapeDrag) {
      ...
      const hit = projectRayOnPlane(
        ...
        e.clientX,
        e.clientY,
        shapeDrag.planeOrigin,
        shapeDrag.planeNormal,
      );
      if (!hit) return;
    ...
    if (marqueeStart) {
      ...
      if (
        ...
        if (selectPickResult?.shapeId && sceneState.editMode !== "edit") {
          ...
          const startHit = projectRayOnPlane(
            camera,
            canvas,
            marqueeStart.x,
            marqueeStart.y,
  ...
  function selectCrossing(
    ...
    const renderer = getRenderer();
    if (!renderer) return;
    const { px, py, pw, ph } = cssRectToDevicePixels(
      cssLeft,
      cssTop,
      cssW,
      cssH,
    ...
    renderer.pickWorldPos(px, py).then((result) => {
      if (result && result.shapeId) {
        ...
        if (
          ...
          sceneState.selectedShapeIds.includes(result.shapeId)
        )
          return;
        if (shift) {
  ...
  function onPointerUp(e: PointerEvent) {
    ...
    if (pushPullDrag) {
      ...
      if (shape) {
        ...
        if (
          ...
          const finalVerts = pushPullPolyEdge(
            pushPullDrag.startVertices,
            pushPullDrag.edgeIdx,
            nx,
            nz,
          ...
          shape.vertices = pushPullDrag.startVertices.map(
            (v) => [...v] as [number, number],
          );
          shape.size = [
            pushPullDrag.startBoundingRadius!,
            shape.size[1],
            shape.size[2],
          ];
          shape.position = [...pushPullDrag.startPos] as Vec3;
      ...
      const endShape = sceneState.shapes.find(
        (s) => s.id === pushPullDrag!.shapeId,
      );
      setRawCursor(
        endShape
    ...
    if (marqueeStart) {
      const marqueeRect = sceneState.marquee
        ? {
            x1: sceneState.marquee.x1,
            y1: sceneState.marquee.y1,
            x2: sceneState.marquee.x2,
            y2: sceneState.marquee.y2,
          }
        : null;
  ...
  function onDblClick(e: MouseEvent) {
    ...
    if (sceneState.activeTool === "polygon") {
      if (
        sceneState.penVertices.length >= 3 &&
        sceneState.drag.phase !== "height"
      ) {
        e.preventDefault();
        e.stopImmediatePropagation();
  ...
  function onContextMenu(e: MouseEvent) {
    // If the user dragged (orbited/panned), suppress the context menu
    if (rightDragged) {
      rightDownPos = null;
      rightDragged = false;
      return;
    ...
    if (!isOverSelectedAABB(e.clientX, e.clientY)) return;

    e.preventDefault();
    e.stopPropagation();

    openContextMenu(e.clientX, e.clientY, buildObjectMenu([...ids]));
  }
